                    ...
                    <div class="card card-custom">
                        ...
                        <div class="card-body pt-0" id="loadTag">
                            <table class="table table-head-custom table-head-bg table-borderless table-vertical-center">
                                ...
                                <tbody>
                                    <template v-for="item in producthot">
                                        <tr :key="'row' + item.id">
                                            <td>
...
export default {
    ...
    created() {
        ...
        axios('/api/producthots').then(res => {
            KTApp.unblock('#loadTag');
            this.producthot = res.data.data
        })
        this.getApi()
    },
    methods: {
        ...
        destroy(id) {
            axios.delete('/api/producthots/' + id).then(res => {
                Swal.fire(
                    "Thành Công!",
                    "sản phẩm đã bị xóa hoàn toàn.",
                    "success"
                )
                this.producthot = this.producthot.filter(item => item.id !== id)
            })
        },
    },
}

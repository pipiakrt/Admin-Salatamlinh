                    ...
                    <div class="card card-custom">
                        ...
                        <div class="card-body pt-0" id="loadTag">
                            <table class="table table-head-custom table-head-bg table-borderless table-vertical-center">
                                ...
                                <tbody>
                                    <template v-for="item in producthightlight">
                                        <tr :key="'row' + item.id">
                                            <td>
                                                <span class="text-dark-75 font-weight-bolder d-block font-size-lg" v-text="item.product.id"></span>
                                            </td>
                                            <td>
                                                <span class="text-dark-75 font-weight-bolder d-block font-size-lg">
                                                    <img class="w-100" :src="item.product.url">
                                                </span>
                                            </td>
                                            <td>
                                                <span class="text-dark-75 font-weight-bolder d-block font-size-lg" v-text="item.product.name"></span>
                                            </td>
                                            <td>
                                                <span class="text-dark-75 font-weight-bolder d-block font-size-lg" v-text="item.product.slug"></span>
                                            </td>
...
export default {
    ...
    created() {
        ...
        axios('/api/producthightlights').then(res => {
            KTApp.unblock('#loadTag');
            this.producthightlight = res.data.data
        })
        this.getApi()
    },
    methods: {
        ...
        async submit(id) {
            if (await this.errors()) {
                ...
                axios.post('/api/producthightlights', params).then((res) => {
                    KTApp.unblockPage();
                    if (res.status == 201) {
                        params.id = res.data._id
                        params.product = this.products.data.find(item => item.id == params.product_id)
                        this.producthightlight.push(params)
        ...
        destroy(id) {
            axios.delete('/api/producthightlights/' + id).then(res => {
                Swal.fire(
                    "Thành Công!",
                    "sản phẩm đã bị xóa hoàn toàn.",
                    "success"
                )
                this.producthightlight = this.producthightlight.filter(item => item.id !== id)
            })
        },
    },
}

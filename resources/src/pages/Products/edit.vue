                ...
                <div class='col-md-12'>
                    <div class="card card-custom gutter-b example example-compact">
                        <div class="card-header">
                            <h3 class="card-title text-capitalize" v-text="'Sản phẩm: ' +name"></h3>
                        </div>
                        <form ref="FormPost">
                                        ...
                                        <div class="col-10">
                                            <validation-provider rules="required" v-slot="{ errors }">
                                                <select v-model="category" class="form-control" style="height: 35px">
                                                    <option :value="category ? category.id : 0" selected v-text="category ? category.name : 'Chọn danh mục'"></option>
    ...
    methods: {
        ...
        async submit(status) {
            if (await this.errors() && await this.checkFromExtend()) {
                ...
                axios.put('/api/products/' + this.post.id, params).then((res) => {
                    KTApp.unblockPage();
                    if (res.status == 200) {
